...
module Network.Search.Solr (SolrInstance(..)
                           ,query
                           ,queryDebug
                           ,add
                           ,update
...
-- verbose/debug version of the above
queryDebug :: SolrInstance -> SearchQuery -> IO (Maybe SearchResult)
queryDebug solr q = do
  print (request)
  response <- sendQueryRequest solr request
  putStrLn "### response ###"
  print response
...
get :: SolrInstance -> SearchQuery -> IO (Result Response_String)
get solr q = do
  res <- sendQueryRequest solr req
  return res                                       
  where req = mkQueryRequest solr (toQueryMap Map.empty q)
...
-- functions to query Solr for documents
